...
이미 KDE Plasma가 설치된 Ubuntu 24.04 데스크톱에서 개발 도구와 공통 dotfile만 적용하려면 개발 데스크톱 모드를 명시합니다.

```bash
LINUX_PROFILE=dev-desktop ./install.sh
```

KDE Plasma 환경 자체를 설치하거나 보강하려면 Linux 데스크톱 설치 모드를 명시합니다.
...
Ubuntu 데스크톱 모드에서는 다음을 확인합니다.

```bash
# 설치 스크립트 문법 확인
bash -n os/linux/dev-desktop/install.sh os/linux/desktop/install.sh
```
