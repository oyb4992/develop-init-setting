...
Personal dotfiles/bootstrap repository for macOS desktop setup, an Ubuntu VPS profile, manual Windows configs, shared shell/editor/terminal assets, and a local n8n Docker service.
...
| Task | Location | Notes |
| --- | --- | --- |
| Top-level bootstrap flow | `install.sh` | macOS runs macOS then common; Linux runs the VPS installer; Windows is manual. |
| Shared shell/editor/terminal config | `os/common/` | Symlinked dotfiles and copied font assets used by the common installer. |
...
| `install_apt_packages` | Bash function | `os/linux/install.sh` | Filters apt package lists against current sources before install. |
...
- The Linux installer assumes apt-based Ubuntu/Debian and targets VPS use only.
...
- Do not apply firewall, shell, Homebrew, quarantine, or sudo side effects without explicit user intent and documented impact.
