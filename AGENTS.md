...
| Task | Location | Notes |
| --- | --- | --- |
| Top-level bootstrap flow | `install.sh` | macOS runs macOS then common; Linux runs Linux only; Windows is manual. |
| Shared shell/editor/terminal config | `os/common/` | Symlinked dotfiles and copied font assets. |
...
| `install_apt_packages` | Bash function | `os/linux/install.sh` | Filters apt package list against current sources before install. |
...
- Linux setup assumes apt-based Ubuntu/Debian. Do not silently broaden it to other package managers.
...
- Do not apply firewall, shell, Homebrew, quarantine, or sudo side effects without documenting the env flag and expected impact.
