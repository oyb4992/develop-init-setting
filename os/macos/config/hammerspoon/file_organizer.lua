...
-- CONFIG에서 설정 가져오기 (기본값 포함)
local function getFileOrganizerConfig()
	...
	return {
		...
		docExts = fo.DOCUMENT_EXTENSIONS or {
			[".pdf"] = true,
			[".doc"] = true,
			[".docx"] = true,
			[".xls"] = true,
...
function M.start()
	...
	local foConfig = getFileOrganizerConfig()

	-- 규칙 정의 영역
	local downloadsRules = {
		-- 규칙 1: 카카오톡 사진을 '년-월' 폴더로 자동 분류하여 이동
		{
			predicate = function(filepath, filename)
				-- 파일명이 "KakaoTalk_Photo_"로 시작하고 이미지 확장자인 경우
				local lowerName = filename:lower()
				return lowerName:match("^kakaotalk_photo_")
					and (lowerName:match("%.jpg$") or lowerName:match("%.jpeg$") or lowerName:match("%.png$"))
			end,
			destination = function(filepath, filename, attr)
				...
				return foConfig.kakaoPhotoDir .. "/" .. dateFolder
			end,
		},
		-- 규칙 2: 일반 이미지 파일 (Pictures/YYYY/MM)
		{
			predicate = function(filepath, filename)
				...
				return ext and foConfig.imageExts[ext:lower()]
			end,
			destination = function(filepath, filename, attr)
				local timestamp = attr.creation or os.time()
		...
		-- 규칙 3: 문서 파일 (Documents/확장자/YYYY/MM)
		{
			predicate = function(filepath, filename)
				local ext = filename:match("^.+(%..+)$")
				return ext and foConfig.docExts[ext:lower()]
			end,
			destination = function(filepath, filename, attr)
				local timestamp = attr.creation or os.time()
