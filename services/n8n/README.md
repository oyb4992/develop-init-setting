...
`.env` 파일을 만들고 다음 두 값을 서로 다른 긴 무작위 문자열로 설정합니다.

```dotenv
N8N_ENCRYPTION_KEY=<32바이트 이상의 무작위 값>
N8N_RUNNERS_AUTH_TOKEN=<별도의 32바이트 이상 무작위 값>
```

대부분의 시스템에서 각 값은 다음 명령으로 생성할 수 있습니다.

```bash
openssl rand -hex 32
```

`.env`는 저장소에서 무시되며 커밋하면 안 됩니다. `N8N_ENCRYPTION_KEY`는 기존 값을 변경하면 저장된 자격 증명을 복호화할 수 없으므로, 이미 실행 중인 환경에서는 원래 값을 유지합니다.
...
* `n8n` 컨테이너를 로컬 포트 **5678**에서 실행합니다.
* 같은 버전의 `n8n-runner` 사이드카에서 JavaScript/Python Code 노드를 격리 실행합니다.
...
이 구성은 SQLite 데이터베이스와 `regular` 모드의 n8n, 외부 Code task runner 사이드카를 사용합니다. JavaScript runner는 현재 워크플로에 필요한 `crypto`, `fs`, `https`, `url` 내장 모듈만 허용합니다. 필요한 모듈이 바뀌면 `n8n-task-runners.json`의 허용 목록도 함께 검토해야 합니다. 워크플로우가 늘어나거나 동시 실행이 많아지는 경우 PostgreSQL 데이터베이스와 queue 모드로 마이그레이션하는 것을 고려할 수 있습니다. 자세한 내용은 n8n 공식 문서를 참고합니다.
...
- [외부 task runner 설정](https://docs.n8n.io/hosting/configuration/task-runners/)
